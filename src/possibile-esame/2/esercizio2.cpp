...
void insert(tree * & result, const int num) {
		if (result == NULL) {
            ...
            result->right = NULL;
            result->left = NULL;
		} else if (result->info > num) { insert(result->left, num); } else { insert(result->right, num); } } void print_tree(const tree * root, int spazio = 0){ if (root != NULL) { spazio++; print_tree(root->right, spazio); for (int i = 1; i < spazio; i++) std::cout<<"  "; std::cout<<root->info<<"\n"; print_tree(root->left, spazio); } } void delete_tree(tree * & root) { if (root != NULL) { delete_tree(root->left);
...
list* estrai_aux(const tree* t, list* lista);
...
list* estrai_aux(const tree* t, list* lista) {
   if (t != NULL) {
      ...
      lista = estrai_aux(t->right, lista);
   }
